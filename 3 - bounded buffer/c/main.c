...
struct BoundedBuffer* buf_new(int size){
    ...
    sem_init(&buf->full,  0, 0); //signals how many occupied slots in buffer
	sem_init(&buf->empty, 0, size); //signals how many empty slots in buffer
...
void buf_push(struct BoundedBuffer* buf, int val){    
    ...
    sem_wait(&buf->empty); //decrements empty, will wait if empty == 0
    pthread_mutex_lock(&buf->mtx); //locking access to resource
    rb_push(buf->buf, val);
    pthread_mutex_unlock(&buf->mtx);
    
    
    // TODO: signal that there are new elements in the buffer
    sem_post(&buf->full); //will signal to increment semaphore full
}
